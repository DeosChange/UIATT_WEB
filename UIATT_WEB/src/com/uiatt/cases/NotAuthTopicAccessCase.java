...
/**
 * 提取2.0 PC 未认证情况下对话题终端页的操作的权限判断以及认证后跳过微信关注页返回来源页校验测试用例
 * author : Change
 * 
 * */
...
public class NotAuthTopicAccessCase {
	...
	public static Logger logger = Logger.getLogger(NotAuthTopicAccessCase.class );
	public static String currentWindowHandler;
	public static Set<String> allWindowHandlers;
	public static String USERNAME;
	public static WebDriverWait wait;
	
	public NotAuthTopicAccessCase(){
		//将基类中初始化的driver值传回本类，这样tearDown()方法的driver就能取到基类中初始化的值了；
		//NotAuthTopicAccessCase.driver=BaseTest.chromeInit(driver);		
	...
	/**
	 * 未认证情况下对话题终端页的操作的权限判断以及认证后返回来源页校验
	 */
	@Test (priority = 1)
	@Parameters({"GLOBAL_PASSWORD","GLOBAL_VALIDCODE"})
	public void notAuthTopicAccess(String GLOBAL_PASSWORD,String GLOBAL_VALIDCODE) {
		//先注册一个跳过认证的邮箱用户，并登陆
		//先进行传值的非空判断并处理
		...
		while(it.hasNext()){
			...
			Utils.sleep(3);
			Assertion.assertEquals(true, OneTopicWebElements.doctAuthBtnIsExist(), "\"医师认证\"按钮未找到，页面不符或元素改变，请检查！");
			logger.info("以未认证身份进入话题详情页，产生权限控制提示，点击'医师认证'按钮");
			Reporter.log("以未认证身份进入话题详情页，产生权限控制提示，点击'医师认证'按钮");
			wait.until(ExpectedConditions.elementToBeClickable(OneTopicWebElements.doct_auth));
			OneTopicWebElements.doct_auth.click();
			Utils.sleep(3);
			//开始认证操作
	 ...
	 * 邮箱退出
	 */
	@Test (priority = 2,dependsOnMethods = { "notAuthTopicAccess" })
	public void logout(){
