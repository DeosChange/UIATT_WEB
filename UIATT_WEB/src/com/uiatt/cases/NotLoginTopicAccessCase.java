...
import com.uiatt.pcWeb.FinalTopicWebElements;
...
/**
 * 提取2.0 PC 未登录情况下对话题终端页的操作的权限判断以及登陆后返回来源页校验测试用例
 * author : Change
 * 
 * */
...
public class NotLoginTopicAccessCase {
	...
	public static Logger logger = Logger.getLogger(NotLoginTopicAccessCase.class );
	public static String currentWindowHandler;
	public static Set<String> allWindowHandlers;
	
	public NotLoginTopicAccessCase(){
		//将基类中初始化的driver值传回本类，这样tearDown()方法的driver就能取到基类中初始化的值了；
		//NotLoginTopicAccessCase.driver=BaseTest.chromeInit(driver);		
		BaseTest.logConf();		
	}
	...
	public void setUp() throws InstantiationException, IllegalAccessException, ClassNotFoundException{
		...
		PageFactory.initElements(driver, FinalTopicWebElements.class);
	...
	/**
	 * 未登录情况下对话题终端页的操作的权限判断以及登陆后返回来源页校验
	 */
	@Test (priority = 1)
	@Parameters({"LOGIN_EMAIL_USER", "GLOBAL_PASSWORD"})
	public void notLoginTopicAccess(String LOGIN_EMAIL_USER, String GLOBAL_PASSWORD) {
		Assertion.assertEquals(true, LoginWebElements.topicBtnIsExist(), "\"话题\"按钮未找到，页面不符或元素改变，请检查！");
		logger.info("点击'话题'按钮");
		Reporter.log("点击'话题'按钮");
		LoginWebElements.topic_btn.click();
		Utils.sleep(2);
		Assertion.assertEquals(true, FinalTopicWebElements.firstTopicIsExist(), "\"正在热议的第一篇话题\"标题链接未找到，页面不符或元素改变，请检查！");
		logger.info("进入话题终端页的正在热议的第一篇话题的标题");
		Reporter.log("进入话题终端页的正在热议的第一篇话题的标题");
		FinalTopicWebElements.first_topic.click();
		Utils.sleep(3);
		...
		while(it.hasNext()){
			...
			logger.info("切换至话题详情页窗口");
			logger.info("driver值检查：  "+driver+" ;当前窗口句柄值检查："+handle);
			Reporter.log("切换至病例详情页窗口");
			Reporter.log("driver值检查：  "+driver+" ;当前窗口句柄值检查："+handle);
			driver.switchTo().window(handle);
			Utils.sleep(3);
			Assertion.assertEquals(true, OneTopicWebElements.loginNowBtnIsExist(), "\"立即登录\"按钮未找到，页面不符或元素改变，请检查！");
			logger.info("以未登录身份进入话题详情页，产生权限控制提示，点击'立即登录'按钮");
			Reporter.log("以未登录身份进入话题详情页，产生权限控制提示，点击'立即登录'按钮");
			OneTopicWebElements.login_now.click();
			Utils.sleep(3);
			...
			if(LoginNowWebElements.wrongUsnorpwNotiIsExist() == true && LoginNowWebElements.wrong_usnorpw_noti.getText().equals("用户名或密码错误！")){
				...
				//ScreenShotPC.screenShot("NotLoginTopicAccessCase.notLoginTopicAccess.wrong_usnorpw_noti");
	    		Assert.fail(LoginNowWebElements.wrong_usnorpw_noti.getText());
			}
			Utils.sleep(2);
			Assertion.assertEquals(true, OneTopicWebElements.tagTitleBtnIsExist(), "\"话题详情页的标签\"按钮未找到，页面不符或元素改变，请检查！");
	 ...
	 * 邮箱退出
	 */
	@Test (priority = 2,dependsOnMethods = { "notLoginTopicAccess" })
	public void logout(){
