...
public class ForgetPWEmailCase {
	...
	public static Logger logger = Logger.getLogger(ForgetPWEmailCase.class );
	
	public ForgetPWEmailCase(){
		//将基类中初始化的driver值传回本类，这样tearDown()方法的driver就能取到基类中初始化的值了；
		//ForgetPWEmailCase.driver=BaseTest.chromeInit(driver);
		BaseTest.logConf();
	}
	...
	public void forgetPWEmail(String LOGIN_EMAIL_USER, String GLOBAL_PASSWORD) {
		logger.info("开始为忘记密码的邮箱账户进行申请重置操作");
		Reporter.log("开始为忘记密码的邮箱账户进行申请重置操作");
		ForgetPWWebElements.forgetAndResetPW(LOGIN_EMAIL_USER, GLOBAL_PASSWORD);
	}
