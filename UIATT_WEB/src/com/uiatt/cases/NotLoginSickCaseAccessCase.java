...
/**
 * 提取2.0 PC 未登录情况下对病例终端页的操作的权限判断以及登陆后返回来源页校验测试用例
 * author : Change
 * 
 * */
@Listeners({com.uiatt.listener.AssertionListener.class})
public class NotLoginSickCaseAccessCase {
	public static WebDriver driver;
	public static Logger logger = Logger.getLogger(NotLoginSickCaseAccessCase.class );
	public static String currentWindowHandler;
	public static Set<String> allWindowHandlers;
	
	public NotLoginSickCaseAccessCase(){
		//将基类中初始化的driver值传回本类，这样tearDown()方法的driver就能取到基类中初始化的值了；
		//NotLoginSickCaseAccessCase.driver=BaseTest.chromeInit(driver);
		BaseTest.logConf();
	}
	...
	/**
	 * 未登录情况下对病例终端页的操作的权限判断以及登陆后返回来源页校验
	 */
	@Test (priority = 1)
	@Parameters({"LOGIN_EMAIL_USER", "GLOBAL_PASSWORD"})
	public void notLoginSickCaseAccess(String LOGIN_EMAIL_USER, String GLOBAL_PASSWORD) {
		...
		while(it.hasNext()){
			...
			Utils.sleep(3);
			Assertion.assertEquals(true, OneSickCaseWebElements.loginNowBtnIsExist(), "\"立即登录\"按钮未找到，页面不符或元素改变，请检查！");
			logger.info("以未登录身份进入病例详情页，产生权限控制提示，点击'立即登录'按钮");
			Reporter.log("以未登录身份进入病例详情页，产生权限控制提示，点击'立即登录'按钮");
			OneSickCaseWebElements.login_now.click();
			Utils.sleep(3);
			...
			if(LoginNowWebElements.wrongUsnorpwNotiIsExist() == true && LoginNowWebElements.wrong_usnorpw_noti.getText().equals("用户名或密码错误！")){
	    		...
	    		//ScreenShotPC.screenShot("NotLoginSickCaseAccessCase.notLoginSickCaseAccess.wrong_usnorpw_noti");
	 ...
	 * 邮箱退出
	 */
	@Test (priority = 2,dependsOnMethods = { "notLoginSickCaseAccess" })
	public void logout(){
