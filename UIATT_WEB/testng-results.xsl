...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="htmlHead">
        <head>
            <title>
                <xsl:value-of select="testng:getVariableSafe($testNgXslt.reportTitle, 'ALLIN PC V2.0 Test Results')"/>
            </title>
    ...
    <xsl:template match="/testng-results">
        ...
        <xsl:result-document href="{testng:absolutePath('overview.html')}" format="xhtml">
            <html xmlns="http://www.w3.org/1999/xhtml">
                <xsl:call-template name="htmlHead"/>
                <body>
                	<h1 align="center">UIATT_WEB V2.0 自动化测试报告</h1>
    ...
    <xsl:template name="testMethodsList">
        ...
        <xsl:for-each select="$methodList">
            ...
            <xsl:variable name="detailsId" select="concat($methodId, '_details')"/>
             <xsl:variable name="errorImgPath" select=" testng:trim(reporter-output/line[7])"/>
                            ...
                            <xsl:choose>
                                <xsl:when test="exception/full-stacktrace">
                                    <pre style="padding: 5px; margin: 0;">
                                        ...
                                        <a  href="{$errorImgPath}" style="text-decoration: none;"><img style="margin: 0 0 0 -160px;height:100;width:100;" src="{$errorImgPath}"></img></a>
                                        <a href="{$errorImgPath}" style="margin: 0 0 0 -140px;">点击查看异常大图</a>
    ...
    <xsl:template name="percentageOverview">
        ...
        <td style="background-color: red; padding: 3px 3px 3px 0;" align="center">
